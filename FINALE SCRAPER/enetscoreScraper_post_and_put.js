...
for(var i = -2; i < 3; i++) {
  ...
  request(url, {jar: true}, function(error, response, body) {
    ...
    $('.default .status').each(function( index ) {
      ...
      if(parseInt(homeGoals) - parseInt(awayGoals) < 0){
        var correctprediction = "2";
      } else if(parseInt(homeGoals) - parseInt(awayGoals > 0)){
        var correctprediction = "1";
      } else if(parseInt(homeGoals) == parseInt(awayGoals)){
        var correctprediction = "X";
      } else{
      ...
      console.log(date + ", " + status + ", " + homeTeam + ", " + awayTeam + ", " + homeGoals + ", " + awayGoals + ", " + matchID);
        ...
        matchData = {
          ...
          action = "updated";
          var id = idToPush[searchId.indexOf(matchID)];
          console.log("id is " + id);

          var options = {
          method: 'PUT',
          url: 'http://localhost:2403/matches/?id=' + id,
          form: matchData       
          }

          console.log(idToPush[searchId.indexOf(matchID)]);
